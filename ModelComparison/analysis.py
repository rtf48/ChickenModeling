...
nn_model = nn_model = MLPRegressor(hidden_layer_sizes=(64, 32),  # Two hidden layers
                        activation='relu',            # ReLU activation
                        solver='adam',                # Optimizer
                        max_iter=100000,
                        )

def train(data_train, labels_train, model):
...
def fill_csv(name, inputs, targets):
    ...
    for label in targets:
        ...
        model = train(data_train, labels_train, nn_model)
...
fill_csv('test', fatty_acids, targets)
